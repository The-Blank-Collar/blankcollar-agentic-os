...
# Optional: if real paperclipai is running natively on the Mac (port 3100),
# note it. Don't fail when it's not — paperclipai is a manual tool.
if curl -fsS --max-time 2 "http://localhost:${PR_PORT}/api/health" >/dev/null 2>&1; then
  ok "Paperclip(real, native) responding on :${PR_PORT}"
else
  printf "\033[1;33m✓\033[0m Paperclip(real, native) not running on :${PR_PORT} (optional — start with: npx paperclipai@latest run)\n"
fi
